...
class Dnn():
    ...
    def run_epoch(self, sess, train_data, dev_data, test_data, epoch):
        ...
        for i, (concat_utter_list, ground_label) in enumerate(minibatches(train_data + dev_data + test_data[:200], self.config.batch_size)):
            input_features = []
    ...
    def run_evaluate(self, sess, test_data):
        ...
        for i, (concat_utter_list, ground_label) in enumerate(
            ...
            labels_distribution = sess.run([self.y_distribution], feed_dict=feed_dict) # for JS_divergence
            ...
            #  JS divergence
            labels_distribution_list = list(labels_distribution)[:][0]
            js_divergence_list = []
            for dist_ele, ground_ele in zip(labels_distribution_list, ground_list):

                first_ele = np.array_split(dist_ele, 3)[0]
                second_ele = np.array_split(dist_ele, 3)[1]
                third_ele = np.array_split(dist_ele, 3)[2]

                input_dist = np.array([first_ele, second_ele, third_ele])

                if ground_ele == 0:  # breakdown
                    ground_truth = np.array([1.0, .0, .0])
                elif ground_ele == 1:  # not breakdown
        ...
        return js_divergence_value, accuracy, precision_X, recall_X, f1_score_X, precision_B_T, recall_B_T, f1_score_B_T
